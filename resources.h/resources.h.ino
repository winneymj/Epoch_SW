#include <Adafruit_GFX.h>
#include <Adafruit_SharpMem.h>
#include <Watch_Menu.h>
#include <Wire.h>
#include <RTCx.h>

#include "icons.h"

#define SCK 19
#define MOSI 18
#define SS 23
#define EXTMODE 11

#define BLACK 0
#define WHITE 1
#define INVERSE 2


Adafruit_SharpMem display(SCK, MOSI, SS);
WatchMenu menu(display);

void mainFunc()
{
  // Clear the display
  display.clearDisplay();
  display.refresh();

  display.setTextColor(BLACK);
  display.setTextSize(1);

  display.print("Here1");
  display.refresh();
}

void initializeMenu()
{
#define NUM_MENUS           3
#define MENU_MAIN_INDEX     0

  menu.setTextSize(0);
  
  menu.initMenu(1);  // Create a menu system with ? menu rows
  
  menu.createMenu(MENU_MAIN_INDEX, 2, PSTR("< MAIN MENU >")); // 3 options
//  menu.createMenu(MENU_SUB_INDEX, 3, PSTR("< SET TIME >")); // 3 options
//  menu.createMenu(MENU_SUB_SUB_INDEX, 2, PSTR("< SLEEP >")); // 2 options

  menu.createOption(MENU_MAIN_INDEX, 0, PSTR("Channels"), menu_stationBitmaps, mainFunc);
  menu.createOption(MENU_MAIN_INDEX, 1, PSTR("Wireless"), menu_wirelessBitmaps, mainFunc);
//  menu.createOption(MENU_MAIN_INDEX, 2, PSTR("Scroll Speed"), menu_speedBitmaps, setDisplaySpeedFunc);
//  menu.createOption(MENU_MAIN_INDEX, 3, PSTR("Exit"), menu_exitBitmaps, backtoSchedule);

//  menu.createOption(MENU_SUB_INDEX, 0, PSTR("2.1st Option"), menu_alarmBitmaps, alarmFunc);
//  menu.createOption(MENU_SUB_INDEX, 1, PSTR("2.2nd Option"), menu_alarmBitmaps, MENU_SUB_SUB_INDEX);
//  menu.createOption(MENU_SUB_INDEX, 2, PSTR("2.Exit Option"), menu_exitBitmaps, MENU_EXIT);
//
//  menu.createOption(MENU_SUB_SUB_INDEX, 0, PSTR("3.1st Option"), menu_wirelessBitmaps, alarmFunc);
//  menu.createOption(MENU_SUB_SUB_INDEX, 1, PSTR("3.Exit Option"), menu_exitBitmaps, MENU_EXIT);

  //!!!! Remove this later after buttons implemented  !!!!!!!
//  handleDefaultSelectOption();
  
  // Setup the pins for the buttons
//  setupButtons();
}


void setup()
{
  Serial.begin(9600);
  Wire.begin();

//  while (!Serial); 

  // Test for a MCP7941x device first.
  uint8_t addressList[] = {RTCx::MCP7941xAddress};

  
  // Autoprobe to find a real-time clock.
  if (rtc.autoprobe(addressList, sizeof(addressList)))
  {
    // Found something, hopefully a clock.
    Serial.println("Autoprobe found ");
  }
  else
  {
    Serial.println("Autoprobe !found ");
  }

    switch (rtc.getDevice())
  {
    case RTCx::DS1307:
      Serial.print("DS1307");
      break;
    case RTCx::MCP7941x:
      Serial.print("MCP7941x");
      break;
    default:
      // Ooops. Must update this example!
      Serial.print("unknown device");
      break;
  }
  Serial.print(" at 0x");
  Serial.println(rtc.getAddress(), HEX);

  // Ensure the oscillator is running.
  rtc.startClock();
  Serial.println("startClock()");

  rtc.setSQW(RTCx::freq1Hz);
  Serial.println("setSQW()");

  pinMode(EXTMODE, OUTPUT); //VCOM Mode (h=ext l=sw)
  digitalWrite(EXTMODE, HIGH); // switch VCOM to external

  // start & clear the display
  display.begin();
  display.clearDisplay();

  // Initialise the display
  initializeMenu();
}

// Font data for Arial 48pt
extern const uint8_t arial_48ptBitmaps[];
extern const FONT_INFO arial_48ptFontInfo;
extern const FONT_CHAR_INFO arial_48ptDescriptors[];

extern const uint8_t arial_48ptWidthPixels;
extern const uint8_t arial_48ptHeightPixels;
extern const uint8_t arial_48ptBitmaps[];

void loop(void) 
{
//  bool animating = menu.updateMenu();
//  // Screen must be refreshed at least once per second
//  display.refresh();
//  if (animating)
//  {
//    delay(20);
//    display.clearDisplay();
//  }
//  else
//  {
     delay(1000);
//  }

  Serial.print("arial_48ptWidthPixels=");
  Serial.println(arial_48ptWidthPixels, DEC);
  Serial.print("arial_48ptHeightPixels=");
  Serial.println(arial_48ptHeightPixels, DEC);

  int arraySize = (arial_48ptWidthPixels * arial_48ptHeightPixels) / 8;
  Serial.print("arial_48ptWidthPixels * arial_48ptHeightPixels=");
  Serial.println(arial_48ptWidthPixels * arial_48ptHeightPixels, DEC);
  Serial.print("arraySize=");
  Serial.println(arraySize, DEC);
  int xpos = 10;
  const byte* bitmap = &arial_48ptBitmaps[1 * arraySize];
  display.drawBitmap(xpos, 10, bitmap, arial_48ptWidthPixels, arial_48ptHeightPixels, BLACK);
  xpos += arial_48ptWidthPixels;

  const byte* bitmap2 = &arial_48ptBitmaps[0 * arraySize];
  display.drawBitmap(xpos, 10, bitmap2, arial_48ptWidthPixels, arial_48ptHeightPixels, BLACK);
  xpos += arial_48ptWidthPixels;
  
  const byte* bitmap3 = &arial_48ptBitmaps[2 * arraySize];
  display.drawBitmap(xpos, 10, bitmap3, arial_48ptWidthPixels, arial_48ptHeightPixels, BLACK);
  xpos += arial_48ptWidthPixels;
  
  const byte* bitmap4 = &arial_48ptBitmaps[4 * arraySize];
  display.drawBitmap(xpos, 10, bitmap4, arial_48ptWidthPixels, arial_48ptHeightPixels, BLACK);
  display.refresh();
}

